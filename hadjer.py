        ...
        for j in range(cols):
            ...
            for x in range(max(0, i - d), min(rows, i + d + 1)):
                for y in range(max(0, j - d), min(cols, j + d + 1)):
                    ...
                    weight = np.exp(-spatial_diff**2 / (2 * sigma_space**2) - intensity_diff**2 / (2 * sigma_color**2))
...
bilateral_filtered_image = bilateral_filter(image_float32, d=9, sigma_color=75, sigma_space=75)
# Apply Bilateral Filter
bilateral_filtered_imageO = cv2.bilateralFilter(image_float32, d=9, sigmaColor=75, sigmaSpace=75)

...
def draw_car(car_x):
    cv2.rectangle(game_window, pt1=(car_x, car_y), pt2=(car_x + car_width, car_y + car_height), color=(0, 255, 0))

# Function to draw obstacles
def draw_obstacles():
    for obstacle in obstacles:
        cv2.rectangle(game_window, pt1=(obstacle[0], obstacle[1]), pt2=(obstacle[0] + obstacle_width, obstacle[1] + obstacle_height), color=(0, 0, 255))
...
def check_collision(car_x):
    for obstacle in obstacles:
        if (
            car_x < obstacle[0] + obstacle_width
...
while True:
    ...
    color_mask, contours = detection.in_range(hsv_frame, lower_bound, upper_bound)

    original = detection.bitwise_and(frame, mask=color_mask)
